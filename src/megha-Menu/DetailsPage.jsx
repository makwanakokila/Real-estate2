            ...
            <div className="container mx-auto px-6 md:px-8 grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-start">
                ...
                >
                    ...
                    <div>
                        ...
                        <ul className="space-y-4">
                            {property.planFeatures?.map((feature, index) => (
                                <motion.li
                                    key={index}
                                    // FIXED: Changed invalid class 'text-gray-1000' to 'text-gray-800'
                    ...
                    <div className="flex flex-wrap gap-4 pt-6">
                        <button
                            onClick={onContactClick}
                            // FIXED: Removed trailing space from className
...
const ContactFormModal = ({ propertyName, onClose }) => {
    // FIXED: Added state management for form inputs
    const [formData, setFormData] = useState({
        fullName: '',
        email: '',
        phone: '',
        enquiryType: 'general_inquiry', // Default value
    });

    // FIXED: Added a change handler for form inputs
    ...
    // FIXED: Added a submit handler for the form
        ...
        >
            ...
            >
                ...
                <p className="text-gray-400 text-center mb-6 text-sm">
                    {/* FIXED: Used the propertyName prop for dynamic content */}
                    Your inquiry for: <span className="font-semibold">{propertyName}</span>
                </p>

                {/* FIXED: Attached the handleSubmit function */}
                <form className="space-y-4" onSubmit={handleSubmit}>
                    ...
                    <div className="relative">
                        ...
                        {/* Custom Arrow */}
                        {/* FIXED: Corrected positioning of the select arrow */}
...
const DetailsPage = () => {
    ...
    useEffect(() => {
        // FIXED: Used strict equality (===) and converted param `id` to a Number for reliable matching.
        const foundProperty = allProjects.find((p) => p.id === Number(id))
        if (foundProperty) {
